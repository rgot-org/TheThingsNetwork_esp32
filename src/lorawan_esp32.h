...
public:
    ...
    static Lorawan_esp32* GetInstance() { return _pLoRaWAN; }
    static uint8_t getPort() { return _port; }
    static TaskHandle_t* TTN_task_Handle;
    static bool _joined;
    bool joined();
    // static void(*messageCallback)(const uint8_t *payload, size_t size, uint8_t
    // port);
